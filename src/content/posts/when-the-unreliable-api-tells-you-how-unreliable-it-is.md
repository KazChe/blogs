...
**The hybrid CLI.** On the four-report demo input, three reports went to the LLM and one was answered from a template, with zero override attempts. It also disagreed with the default engine on two of the four inputs: it filed "upload is broken" as partial at 0.92, the same literal reading that misfiled the all-caps upload complaint in the corpus, and the indemnity-clause report as actionable at 0.83. Same inputs, and two models that draw the boundaries in different places.
